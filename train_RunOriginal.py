...
#csv_file = r"C:\Users\Eliana\Downloads\Fitzpatrick17kImages\FitzpatrickData3SkinType.csv"
# csv_file = r"C:\Users\Eliana\Downloads\Fitzpatrick17kImages\FitzpatrickDataAddDataToType6.csv"
...
images_folder_ddi = r"C:\Users\Eliana\Downloads\ddidiversedermatologyimages"
...
parser.add_argument('--sample', action='store_true', default=False, help='Sample usage flag')  # This will be True if specified, otherwise False
parser.add_argument('--augAll', action='store_false', default=False, help='Augment data flag')  # This will be True if specified, otherwise False
parser.add_argument('--fix_tests', action='store_false', default=False, help='if test data is fixed')  # This will be True if specified, otherwise False
parser.add_argument('--run_original', action='store_false', default=False, help='if test data is fixed')  # This will be True if specified, otherwise False
...
parser.add_argument('--trainOnly', type=str, default="", help='only use F1 for training')
parser.add_argument('--trainOnlyPercent', type=int, default=100, help='only use the percent of F1 for training')
...
if(args.trainOnly == ""): # if not train only F1 or F2,.....
    ...
    args.testName = "T_EffNet_" + "trainonly_" + str(args.trainOnly) + "_fix_tests_" + str(args.fix_tests) + "_smpleS_" + str(args.sampleSize) + "_Percent_" + str(args.trainOnlyPercent) + "_TestSizeF6"  + filename_datetime_string
...
print(args.dupCount)
print("fix_tests")
print(args.fix_tests)
print("run_original")
print(args.run_original)
...
while (numRuns < args.totalNumRuns):
    # output_file_path = os.path.join(args.output_folder, 'output.txt')
    output_file_path = os.path.join(args.output_folder + args.testName + str(numRuns) + '.txt')

    with open(output_file_path, 'w') as output_file:
    # with open(args.output_folder, 'w') as output_file:
        ...
        if (args.fix_tests):
            fitzpatrick1_test = fitzpatrick1.sample(n=numberofTestingSample, random_state=2)  # Setting a random_state for reproducibility
            fitzpatrick2_test = fitzpatrick2.sample(n=numberofTestingSample, random_state=2)  # Setting a random_state for reproducibility
            fitzpatrick3_test = fitzpatrick3.sample(n=numberofTestingSample, random_state=2)  # Setting a random_state for reproducibility
            fitzpatrick4_test = fitzpatrick4.sample(n=numberofTestingSample, random_state=2)  # Setting a random_state for reproducibility
            fitzpatrick5_test = fitzpatrick5.sample(n=numberofTestingSample, random_state=2)  # Setting a random_state for reproducibility
            fitzpatrick6_test = fitzpatrick6.sample(n=numberofTestingSample, random_state=2)  # Setting a random_state for reproducibility
        else:
        ...
        if (args.trainOnly == ""):
            ...
            if(not args.run_original): # if run original, do not augment
            ...
            # else:
            #     # no sample
            #     resultMetadata = pd.concat([fitzpatrick1_rest, fitzpatrick2_rest, fitzpatrick3_rest, fitzpatrick4_rest, fitzpatrick5_rest, fitzpatrick6_rest], ignore_index=True)
            resultMetadata = pd.concat([fitzpatrick1S, fitzpatrick2S, fitzpatrick3S, fitzpatrick4S, fitzpatrick5S, fitzpatrick6S], ignore_index=True)

        else: # if only train F1 or F2,.....
        ...
        resultMetadata['full_path'] = resultMetadata[imageColName].apply(
            lambda x:
                ...
                os.path.join(images_folder_ddi, x) if x.endswith(".png") else
