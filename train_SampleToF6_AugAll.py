...
while (numRuns < args.totalNumRuns):
    ...
    # with open(args.output_folder, 'w') as output_file:
        ...
        print (f"Length of resultMetadata without augmentation: {len(resultMetadata)}")

        if (len(augmented_Sample) > 0) :
            resultMetadata = pd.concat([resultMetadata, augmented_Sample], ignore_index=True)

        print (f"Length of resultMetadata with augmentation: {len(resultMetadata)}")
